...
// ⚡ Bolt: Extracting components outside the render loop prevents ReactMarkdown
// from unnecessarily unmounting and remounting all nodes on every keystroke,
// which previously caused significant input lag.
const markdownComponents: Components = {
  p: ({ node, ...props }) => (
    <p
      className="mb-4 last:mb-0 leading-relaxed text-purple-50/90"
      {...props}
    />
  ),
  strong: ({ node, ...props }) => (
    <strong
      className="font-bold text-purple-200 drop-shadow-[0_0_8px_rgba(192,132,252,0.6)]"
      {...props}
    />
  ),
  em: ({ node, ...props }) => (
    <em
      className="italic text-amber-100/80 font-serif tracking-wide"
      {...props}
    />
  ),
  h1: ({ node, ...props }) => (
    <h1
      className="text-2xl font-cinzel text-transparent bg-clip-text bg-gradient-to-r from-purple-200 to-amber-100 mb-4 mt-6 border-b border-purple-500/30 pb-2"
      {...props}
    />
  ),
  h2: ({ node, ...props }) => (
    <h2 className="text-xl font-cinzel text-purple-200 mb-3 mt-5" {...props} />
  ),
  h3: ({ node, ...props }) => (
    <h3 className="text-lg font-cinzel text-purple-300 mb-2 mt-4" {...props} />
  ),
  code: ({ node, className, ...props }: any) => {
    ...
    return isInline ? (
      <code
        className="font-mono text-xs bg-purple-900/30 px-1.5 py-0.5 rounded text-green-300 border border-green-500/20"
        {...props}
      />
    ) : (
  ...
  pre: ({ node, ...props }) => (
    <pre
      className="font-mono text-xs bg-black/60 p-4 rounded-lg overflow-x-auto mb-4 border border-green-500/20 shadow-[0_0_15px_rgba(34,197,94,0.1)] backdrop-blur-sm"
      {...props}
    />
  ),
  blockquote: ({ node, ...props }) => (
    <blockquote
      className="border-l-2 border-amber-500/50 pl-4 italic text-slate-400 my-4 bg-amber-500/5 py-2 pr-2 rounded-r"
      {...props}
    />
  ),
  ul: ({ node, ...props }) => (
    <ul
      className="list-disc pl-5 mb-4 space-y-1 text-purple-100/80"
      {...props}
    />
  ),
  ol: ({ node, ...props }) => (
    <ol
      className="list-decimal pl-5 mb-4 space-y-1 text-purple-100/80"
      {...props}
    />
  ),
  a: ({ node, ...props }) => (
    <a
      className="text-cyan-300 hover:text-cyan-100 underline underline-offset-2 decoration-cyan-500/50 hover:decoration-cyan-300 transition-colors"
      {...props}
    />
  ),
};
...
const App: React.FC = () => {
  ...
  const handleResetMemory = () => {
    if (
      window.confirm(
        "Are you sure you want to wipe Clio's memory? This cannot be undone.",
      )
    ) {
