...
const Navigation: React.FC<NavigationProps> = ({ currentScreen, onScreenChange }) => {
  return (
    <nav className="fixed bottom-0 w-full bg-slate-950/90 backdrop-blur-xl border-t border-purple-900/50 z-50 px-4 py-3 pb-safe">
      <div className="max-w-md mx-auto flex justify-between items-center">
        <NavButton
          active={currentScreen === ScreenState.CHAT}
          onClick={() => onScreenChange(ScreenState.CHAT)}
        ...
        />
        <NavButton
          active={currentScreen === ScreenState.CUSTOMIZATION}
          onClick={() => onScreenChange(ScreenState.CUSTOMIZATION)}
        ...
        />
        <NavButton
          active={currentScreen === ScreenState.KNOWLEDGE}
          onClick={() => onScreenChange(ScreenState.KNOWLEDGE)}
          icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.332A48.36 48.36 0 0012 9.75c-2.551 0-5.056.2-7.5.582V21M3 21h18M12 6.75h.008v.008H12V6.75z" /></svg>}
          label="Lore"
        />
        <NavButton
          active={currentScreen === ScreenState.SETTINGS}
          onClick={() => onScreenChange(ScreenState.SETTINGS)}
...
const NavButton: React.FC<NavButtonProps> = ({ active, onClick, icon, label }) => {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center gap-1 p-2 transition-all duration-300 ${active ? 'text-amber-200' : 'text-purple-400 hover:text-purple-200'} relative group`}
    >
      ...
      <span className={`text-[10px] font-cinzel tracking-wider uppercase transition-all duration-300 ${active ? 'opacity-100 font-bold' : 'opacity-60'}`}>
        {label}
      </span>
      {active && (
        <div className="absolute -bottom-1 w-1/2 h-0.5 bg-gradient-to-r from-transparent via-amber-400 to-transparent"></div>
      )}
    </button>
  );
};
